...
# # 📊 1️⃣ Fix: Use `Agg` mode to prevent Matplotlib GUI errors
# matplotlib.use('Agg')
...
#     # Load stream mapping from pickle file
#     with open("stream_mapping.pkl", "rb") as f:
#         stream_mapping = pickle.load(f) # Load stream mapping
...
#     student_df = student_df.dropna()  # Drop rows with missing values

#     # Sort by actual score & take top 10 students
#     top_students = student_df.sort_values(by="Total Score", ascending=False).head(10)
#     features = top_students[['Age', 'Gender', '10th %', '12th %', 'Stream']].values  # Convert to NumPy array
#     features = features.reshape(-1, 5)  # Ensure 2D shape (n_samples, n_features)
...
#     #predicted_scores = model.predict(top_students[['Age', 'Gender', '10th %', '12th %', 'Stream']])
