...
QUESTIONS_FILE= "CODE_QUESTION.csv"
...
def extract_score(feedback):
    ...
    return min(max(int(match.group(1)), 0), 10) if match else 2 # Default to 2 if no score found
...
def process_csv(file_path):
    ...
    from app import store_scores 
    global evaluated_data
    evaluated_data.clear()  # Reset evaluated data
...
def upload_file():
    ...
    # Load questions directly inside this function
    try:
        df = pd.read_csv(QUESTIONS_FILE)  # Read CSV file
        df.columns = df.columns.str.strip()  # Remove spaces in column names

        if "Question" in df.columns:
            questions = df["Question"].dropna().tolist()
            #questions = df[df["type"].str.lower() == "code"]["question"].tolist()
    except Exception as e:
