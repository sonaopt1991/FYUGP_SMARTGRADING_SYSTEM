...
# Load data for MCQ and Essay test
credentials_df = pd.read_csv("USERS.csv")
questions_df_mcq = pd.read_excel("Book2.xlsx")
questions_df_essay = pd.read_csv('essayquestion.csv')

# Preprocess Functions - Removed since we don't need lemmatization for BERT
def get_bert_embedding(text):
    inputs = bert_tokenizer(text, padding=True, truncation=True, return_tensors="pt", max_length=512)
    with torch.no_grad():
...
def compute_bert_similarity(model_answers, student_answer):
    ...
    for model_answer in model_answers:
        ...
        similarity = torch.cosine_similarity(student_embedding, model_embedding).item()
...
def grade_essay(filename):
    ...
    for idx, row in questions_df_essay.iterrows():
        ...
        transformer_scores = compute_bert_similarity([model_answer], student_answer)
